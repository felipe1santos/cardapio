...
interface ConfigRow {
  ...
  banner_promo_foco_y: number | string | null
  telefone: string
  endereco: string
...
export async function atualizarConfigLoja(supabase: SupabaseClient, restauranteId: string, patch: ConfigLojaPatch): Promise<ConfigLoja> {
  ...
  if (patch.bannerPromoFoco !== undefined) {
    row.banner_promo_foco_x = patch.bannerPromoFoco.x
    row.banner_promo_foco_y = patch.bannerPromoFoco.y
  }
