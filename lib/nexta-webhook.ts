/**
 * Processamento do webhook de eventos do Nexta, compartilhado entre as rotas
 * `/api/nexta/webhook/[token]` e `/api/nexta/webhook/[token]/[...evento]`.
 *
 * O Nexta anexa um sufixo à URL registrada (ex.: `/deliveryUpdate`), então a URL que a
 * gente cadastra com o suporte recebe os POSTs em `{url}/deliveryUpdate`. As duas rotas
 * delegam para cá com o mesmo token — o sufixo em si não muda o processamento (o tipo do
 * evento vem no corpo, em `event.type`).
 *
 * SERVER-ONLY — usa o service_role e importa `lib/nexta.ts` (node:crypto).
 */
 ...
 * reenvio.
 */
export async function processarWebhookNexta(request: Request, token: string): Promise<NextResponse> {
