...
ok('   sem escolher Retirada/Entrega o botão Abrir fica desabilitado', await pAt.getByTestId('balcao-abrir').isDisabled())
await pAt.getByTestId('balcao-modalidade-retirada').click() // c771515: escolha antes do nome
...
await pAt.getByTestId('card-balcao').click()
await pAt.getByTestId('balcao-novo').click()
await pAt.getByTestId('balcao-modalidade-entrega').click() // c771515: escolha antes do nome
...
await pAt.waitForTimeout(2500) // busca do CEP (ViaCEP) termina antes de digitar o resto
...
await pAt.getByTestId('entrega-estado').fill('ES')
...
ok('7/8. pedido telefônico com dados de entrega completos pelo mesmo card', cEnt.entrega && cEnt.entrega_rua === 'Rua de Demonstração' && cEnt.entrega_numero === '100'
  && cEnt.entrega_bairro === 'Centro' && cEnt.entrega_cep === '29000000' && cEnt.entrega_complemento === 'Casa 2' && cEnt.entrega_cidade === 'Cidade Demo/ES'
...
// O card ficou só com nome, preço e itens (origem e atendimento nas etiquetas de cima).
// Senha, comanda e quem lançou passaram para os Detalhes: cada caso é conferido lá.
ok('38. Kanban: card sem o texto repetido "PDV · …" / "Salão · …"', !/(PDV|Salão) · /.test(kanban))
const abertoMaisRecente = (where, p = []) => um(`select p.numero, p.criado_por_nome, c.senha, c.numero comanda from pedidos p left join comandas c on c.id = p.comanda_id
  where p.restaurante_id=$1 and p.status in ('recebido','preparando','pronto') and ${where} order by p.criado_em desc limit 1`, [loja, ...p])
...
const pBal = await abertoMaisRecente(`p.canal='balcao' and p.tipo='retirada'`)
const dBal = await detalhesDo(pBal.numero)
ok('38. Detalhes do balcão: PDV, RETIRADA e "Senha N · Lançado por …"', /PDV/.test(dBal.etiquetas) && /RETIRADA/.test(dBal.etiquetas)
  && dBal.lancamento === `Senha ${pBal.senha} · Lançado por ${pBal.criado_por_nome}`, `${dBal.etiquetas} | ${dBal.lancamento}`)
const pEntM = await abertoMaisRecente(`p.comanda_id=$2`, [cEnt.id])
const dEntM = await detalhesDo(pEntM.numero)
ok('38. Detalhes da entrega manual: PDV, ENTREGA e a senha', /PDV/.test(dEntM.etiquetas) && /ENTREGA/.test(dEntM.etiquetas)
  && dEntM.lancamento.startsWith(`Senha ${pEntM.senha}`), `${dEntM.etiquetas} | ${dEntM.lancamento}`)
const pSal = await abertoMaisRecente(`p.id=$2`, [lg.json.pedidoId])
const dSal = await detalhesDo(pSal.numero)
ok('38. Detalhes do salão: SALÃO, MESA, a mesa e "Comanda N · Lançado por <garçom>"', /SALÃO/.test(dSal.etiquetas) && /MESA/.test(dSal.etiquetas) && dSal.etiquetas.includes(livre.nome)
  && dSal.lancamento === `Comanda ${pSal.comanda} · Lançado por ${pSal.criado_por_nome}`, `${dSal.etiquetas} | ${dSal.lancamento}`)
...
const pPdvM = await abertoMaisRecente(`p.id=$2`, [lp.json?.id])
const dPdvM = pPdvM ? await detalhesDo(pPdvM.numero) : { etiquetas: '', lancamento: '' }
ok('38. Detalhes quando o caixa lança na mesa: PDV, MESA, a mesa e a comanda', lp.status === 201 && /PDV/.test(dPdvM.etiquetas) && /MESA/.test(dPdvM.etiquetas)
  && dPdvM.etiquetas.includes(M1.nome) && dPdvM.lancamento.startsWith(`Comanda ${pPdvM?.comanda}`), `${dPdvM.etiquetas} | ${dPdvM.lancamento}`)
