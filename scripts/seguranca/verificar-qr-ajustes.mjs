...
import { USU, exigirLojaIsolada } from './e2e-ambiente.mjs'
...
// ed43021: a aba virou dois blocos — "QR das mesas · cardápio do salão" e "QR do delivery · vitrine de entrega".
await page.getByText('QR das mesas · cardápio do salão').waitFor({ timeout: 15000 })
...
ok('o QR da vitrine fica identificado como delivery', /QR do delivery · vitrine de entrega/.test(texto) && /Este NÃO vai na mesa/.test(texto))
