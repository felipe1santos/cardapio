...
// Fila da cozinha no modo de sempre: roteamento por função (impressão, opcional) desligado.
await db.query(`update restaurantes set pdv_v2=false, impressao_automatica=true, impressao_cozinha_por_funcao=false where id=$1`, [loja])
...
// 0094: telefone normalizado (55 + DDD + número) e vinculado ao cadastro DA LOJA.
ok('telefone normalizado na comanda', comandas[1].cliente_telefone === '5527999990001')
ok('balcão sem taxa de serviço (loja tem 10%)', comandas.every((c) => Number(c.taxa_servico_percentual) === 0))
ok('telefone do balcão vincula um cadastro da loja, sem duplicar', Number((await um(`select count(*) n from clientes where restaurante_id=$1 and telefone like '%999990001'`, [loja])).n) === 1)
...
await pa.getByTestId('fechar-modal').getByText('Aguardando aceite').waitFor()
...
// Pago e sem pendências: o fechamento confirma na mesma tela (saldo coberto).
await pa.getByTestId('fechar-modal').waitFor()
await pa.waitForFunction(() => !(document.querySelector('[data-testid="fechar-confirmar"]'))?.disabled)
...
// 0094: mesa livre só abre com o nome do cliente.
await pa.getByTestId('mesa-nome').fill('Cliente da Mesa')
...
ok('mesa aberta com o nome do cliente, com taxa padrão 10%', !!cm && cm.tipo === 'mesa' && Number(cm.taxa_servico_percentual) === 10)
...
await pa.goto(`${BASE}/admin/pdv`, { waitUntil: 'networkidle' })
// 0095: fechada, a mesa fica EM LIMPEZA até alguém liberar.
ok('mesa fechada fica em limpeza (não volta livre sozinha)', (await pa.getByRole('button', { name: new RegExp(mesaLivre.nome) }).first().innerText()).includes('Toque p/ liberar'))
await pa.getByRole('button', { name: new RegExp(mesaLivre.nome) }).first().click()
await pa.getByTestId('mesa-liberar').click()
await pa.waitForFunction((n) => [...document.querySelectorAll('button')].some((b) => b.innerText.includes(n) && b.innerText.includes('Toque p/ abrir')), mesaLivre.nome, { timeout: 15000 })
ok('liberada, a mesa volta a aparecer livre', (await pa.getByRole('button', { name: new RegExp(mesaLivre.nome) }).first().innerText()).includes('Toque p/ abrir'))
